...
def walkforward_accuracy(two_days_ago, yesterday, today):
...
def organize_upload(today_close, tomorrow_predicted_close):
	dict_to_upload = {
				'Date': global_date_today_str,
				'close': today_close,
				'tomorrow_predicted_close': tomorrow_predicted_close,
				'prior_day_prediction_accuracy': None
			}
...
def main():
	try:
		two_days_ago_query = "select * from btc_trading_v025 where date like\
		'{0}';".format(global_date_twodays_str)
		...
		upload_data = organize_upload(today_close_price, prediction)
